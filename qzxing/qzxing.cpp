...
#if 1
        HybridBinarizer *binz = new HybridBinarizer(imageRef);
#else
                                  ...
                                  const double width, const double height)
{
    ...
    QString proto=imageUrl.scheme();
    imagePath = imagePath.trimmed();

    qDebug() << imageUrl;

    if (proto=="file" || proto=="qrc") {
        ...
        img.load(imageUrl.path());
    } else if (proto=="image") {
        qDebug() << imageUrl.path();
        qDebug() << imageUrl.host();
        qDebug() << imageUrl.fileName();
        QQmlEngine *engine = QQmlEngine::contextForObject(this)->engine();
        QQmlImageProviderBase* imageProviderBase = engine->imageProvider(imageUrl.host());

        if (!imageProviderBase)
            qWarning() << "Image provider not found: " << imageUrl.host();

        QQuickImageProvider* imageProvider = dynamic_cast<QQuickImageProvider*>(imageProviderBase);

        QSize rtmp;

        img=imageProvider->requestImage(imageUrl.fileName(), NULL, rtmp);
        if (img.isNull())
            qWarning() << "Failed to get requested image from provider " << imageProvider->imageType();
    } else {
